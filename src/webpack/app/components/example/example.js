...
import missingThumbnail from '../../assets/images/missing-thumbnail.jpg';
import './example.css';
...
export const Example = ({
  ...
  if (lang === 'html') {
    ...
    try {
      ...
      livePreview = <ExampleError err={err} />;
    }
  }
  ...
  const codeBoxParams = getParameters(
    lang === 'html' ? getStaticParams(title, editorCode) : getReactParams(title, editorCode, scope, lang)
  );
  const fullscreenLink = `${
    loc.pathname.replace(/\/$/, '') + (loc.pathname.endsWith(source) ? '' : `/${source}`)
  }/${slugger(title)}`;
  ...
  return (
    <div className="ws-example">
      ...
      {isFullscreen ? (
        ...
        <div id={previewId} className={css(className, isFullscreen ? 'ws-preview-fullscreen' : 'ws-preview')}>
          {livePreview}
        </div>
      )}
      <ExampleToolbar
