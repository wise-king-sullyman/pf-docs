...
function writeIndex() {
  const stringifyRoute = ([route, pageData]) =>
    `'${route}': {\n    ${Object.entries(pageData)
      ...
      .concat(
        `Component: () => import(/* webpackChunkName: "${route.substr(1)}/index" */ '.${route}')`
      )
      .join(',\n    ')}\n  }`;
...
module.exports = {
  ...
  watchMD() {
    globs.props.forEach(({ glob, ignore }) => {
      const watcher = chokidar.watch(glob, { ignored: ignore, ignoreInitial: true });
      watcher.on('add', sourcePropsFile);
      watcher.on('change', sourcePropsFile);
    });

    globs.md.forEach(({ glob, source, ignore }) => {
      const watcher = chokidar.watch(glob, { ignored: ignore, ignoreInitial: true });
      const onMDFileChange = file => {
        sourceMDFile(file, source, 'start');
        writeIndex();
      };

      watcher.on('add', onMDFileChange);
      watcher.on('change', onMDFileChange);
    });
  }
};
