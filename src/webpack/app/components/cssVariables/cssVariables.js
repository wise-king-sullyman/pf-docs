  ...
  <div>
    ...
    {values.map((entry, index) => (
      ...
      >
        ...
        <span style={{ paddingLeft: '16px' }}>{entry}</span>
      </div>
    ))}
  </div>
);
...
export class CSSVariables extends React.Component {
  constructor(props) {
    ...
    this.prefix = typeof props.prefix === 'string' ? [props.prefix] : props.prefix;
            ...
            <div key={`${rowKey}_1`} className="pf-l-flex pf-m-space-items-sm">
              {isColorRegex.test(value) && (
                <div key={`${rowKey}_2`} className="pf-l-flex pf-m-column pf-m-align-self-center">
                  <span className="ws-color-box" style={{ backgroundColor: value }} />
                </div>
              )}
