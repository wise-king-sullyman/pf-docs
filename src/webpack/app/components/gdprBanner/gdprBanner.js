...
export const GdprBanner = () => {
  const initialBannerOpen = !window?.localStorage?.getItem('gdpr-banner-closed');
  const [isBannerOpen, setBannerOpen] = React.useState(initialBannerOpen);

  const closeBanner = () => {
    window?.localStorage?.setItem('gdpr-banner-closed', 'true');
    setBannerOpen(false);
  };
    ...
    <div className="ws-gdpr-banner-container pf-l-flex">
      <div id="ws-gdpr-banner" className="pf-l-flex pf-u-py-md pf-m-align-items-center">
        <p id="ws-gdpr-banner-text" className="pf-u-ml-xl">
          ...
          may disable them are set out in our <a href="//www.redhat.com/en/about/privacy-policy">Privacy Statement</a>.
