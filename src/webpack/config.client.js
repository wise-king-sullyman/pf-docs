...
const { getHtmlWebpackPlugins } = require('./config.plugins');

const { _PF_DOCS_PACKAGE_PWD } = global;
...
try {
  ...
  pfDir = path.join(_PF_DOCS_PACKAGE_PWD, 'dist');
}

const staticDir = path.join(_PF_DOCS_PACKAGE_PWD, 'static/');
...
const clientConfig = async (env, argv) => {
  ...
  return {
    ...
    optimization: {
      ...
      minimizer: [
        new TerserPlugin({
          cache: path.join(_PF_DOCS_PACKAGE_PWD, '.cache/terser'),
    ...
    plugins: [
      ...
      new CopyPlugin({
        patterns: [
          ...
          {
            from: path.dirname(require.resolve('theme-patternfly-org/versions.json')),
            to: 'versions.json'
          },
...
module.exports = async (env = {}, argv) =>
  merge(baseConfig(env, argv), await clientConfig(env, argv));
